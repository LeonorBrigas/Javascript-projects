// Get your shorts on - this is an array workout!
    ...
    // need to define the starting value for total (line 69)
    ...
    // need to change to array in order to use map method

    // ways to do it [...categories.querySelectorAll("a")]
    // OR Array.from(categories.querySelectorAll("a"))

    // comment to be able to see in the console without error
    ...
    const ordererAlpha = people.sort((a, b) => {
      const [aLast, aFirst] = a.split(", ");
      const [bLast, bFirst] = b.split(", ");
      return aLast > bLast ? 1 : -1;
    });

    console.log(ordererAlpha);

    // 8. Reduce Exercise
    // Sum up the instances of each of these
    const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck' ];

    const transf = data.reduce((obj, i) =>{
      if(!obj[i]) {
        obj[i] = 0;
      }
      obj[i]++;
